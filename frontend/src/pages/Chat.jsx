...
const SECRET_KEY = import.meta.env.VITE_SECRET_KEY || "fallback";
...
function Chat() {
  ...
  const getAIMessage = async () => {
    try {
      const res = await fetch(`${BACKEND_URL}/ai-message`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          interests: "travel, music, movies",
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-gray-900 to-black text-white">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((m) => {
          const isMe = m.sender === user._id;
          const decrypted = decryptMessage(m.message); // ✅ msgText → decrypted
        ...
        {preview && (
          <div className="flex justify-end">
            <img src={preview} alt="preview" className="w-20 rounded" /> {/* ✅ alt fix */}
          </div>
        )}
      ...
      {/* INPUT BAR */}
      <div className="p-3 flex gap-2 border-t border-gray-700 items-center">
        <input
          type="file"
          hidden
          id="fileInput"
        ...
        <input
          className="flex-1 p-2 bg-gray-800 rounded-full px-4 outline-none focus:ring-2 focus:ring-pink-500"
          value={text}
        ...
        <button
          onClick={sendMessage}
          className="bg-pink-500 px-4 py-2 rounded-full hover:scale-105 transition font-semibold"
        >
          ➤
        </button>
      </div>
