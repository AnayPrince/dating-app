...
export default function Dashboard() {
  ...
  const navigate = useNavigate();
  const socketRef = useRef(null); // ✅ useRef use karo - re-render nahi hoga
  ...
  useEffect(() => {
    ...
    // ✅ Production URL use hogi, localhost nahi
  ...
  useEffect(() => {
    ...
    socket.on("matchAlert", (data) => {
      toast.success("🎉 It's a Match!");
      setUsers((prev) => prev.filter((u) => u._id !== data.fromUser));
      navigate("/matches");
    });
  ...
  }, [userId, navigate]); // ✅ dependencies sahi hain
  ...
  const enable2FA = async () => {
    ...
    try {
      const res = await API.post("/auth/enable-2fa", { userId: storedUser._id });
      setQr(res.data.qr);
      setShowQR(true);
      toast.success("Scan QR in Google Authenticator 📱");
    } catch (err) {
  ...
  const likeUser = async (id) => {
    try {
      const res = await API.post("/like", { toUser: id });
      ...
      socketRef.current?.emit("likeUser", { toUser: id, fromUser: userId });
      setUsers((prev) => prev.filter((u) => u._id !== id));

      if (res.data.match) {
        socketRef.current?.emit("matchFound", { toUser: id, fromUser: userId });
      }
    } catch (err) {
      console.log(err);
    }
  ...
  ====================== */
  if (loading) {
    return <h2 className="text-center mt-10 text-white">Loading users...</h2>;
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-purple-900 to-pink-900 flex flex-col items-center py-10">
      ...
      <button
        onClick={() => (!showQR ? enable2FA() : setShowQR(false))}
        className="mb-6 bg-green-500 px-6 py-2 rounded-full text-white font-semibold hover:scale-105"
      >
        🔐 {showQR ? "Hide QR" : "Enable 2FA"}
      </button>
      ...
      {showQR && qr && (
        <div className="bg-black p-4 rounded-xl text-center mb-6">
          <p className="text-white mb-2">Scan this QR in Google Authenticator</p>
          <img src={qr} alt="QR Code" className="mx-auto w-40" />
          <p className="text-gray-400 text-sm mt-2">
            After scanning, logout & login again 🔐
          </p>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 px-4">
        {users.map((u) => (
          <div
            key={u._id}
            className="relative w-80 bg-white/10 backdrop-blur-2xl border border-white/20 rounded-3xl p-6 shadow-2xl hover:scale-105 hover:shadow-pink-500/30 transition duration-300"
          >
            <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-pink-500 to-purple-500 opacity-10 blur-xl pointer-events-none"></div>

            <div className="relative text-center z-10">
              <div className="w-20 h-20 mx-auto mb-4 rounded-full bg-gradient-to-r from-pink-500 to-purple-500 flex items-center justify-center text-2xl font-bold text-white">
                {u.name.charAt(0)}
              </div>
            ...
            <div className="flex justify-between mt-6 relative z-10">
              <button
                ...
                className="bg-gradient-to-r from-pink-500 to-red-500 px-5 py-2 rounded-full shadow-lg hover:scale-110 transition text-white font-semibold"
              >
                ❤️ Like
              </button>
              <button
                onClick={() => setUsers((prev) => prev.filter((x) => x._id !== u._id))}
                className="bg-gray-800 text-white px-5 py-2 rounded-full hover:bg-gray-700 transition"
              >
                ❌ Skip
              </button>
            </div>
